...
<head>
  ...
  <title>renvoye-<%=pointsVente.getNom()%></title>
</head>
<style>
  .error{
    color: white;
...
<div class="content">
  ...
  <div class="main">
    <div class="table-main">
      <h1>Renvoye-<%=pointsVente.getNom()%></h1>
    </div>
    <div class="table-main">
      <table>
        <tr>
        ...
        <tr>
          <form action="<%=request.getContextPath()%>/user/renvoye/add" method="post" modelAttribute="renvoye">
            <input type="hidden" name="stock" value="<%=stockPoints1.getId()%>"/>
            <input type="hidden" name="receptionTransfere.id" value="<%=stockPoints1.getReceptionTransfere().getId()%>"/>
            <td><%=stockPoints1.getReceptionTransfere().getDate()%></td>
            <td><%=stockPoints1.getReceptionTransfere().getTransfere().getStockMagasin().getLaptop().affiche()%></td>
            <td><%=stockPoints1.getReceptionTransfere().getQuantite()%></td>
            <td><%=stockPoints1.getReceptionTransfere().getEtat().getNom()%></td>
