...
public class VenteController {
    ...
    public String handleItemException(RedirectAttributes redirectAttributes, Model model, Exception ex) {
        ...
        if (ex instanceof BadRequestException) {
            ...
            if (e.getFunction() != null) {
                ...
                value = "redirect:/user/stock/" + e.getFunction();
            } else {
    ...
    public String listeVente(@ModelAttribute Vente vente, Model model, @RequestParam(defaultValue = "0") int min, @RequestParam(defaultValue = "0") int max, @RequestParam(defaultValue = "") String reference)throws Exception{
        ...
        model.addAttribute("vente",vente.listeVente(customUser.getUtilisateur().getId_points_vente(),reference,min,max));
        return "liste-vente";
    }
    @GetMapping("/")
    public String stock(Model model)throws Exception{
        ...
        model.addAttribute("points-vente",customUser.getUtilisateur().getId_points_vente());
        model.addAttribute("stock",new StockPoints().findQuery(dataSource.getConnection(),"select * from v_stock_points where quantite_stock_point > 0 and id_points_vente="+customUser.getUtilisateur().getId_points_vente().getId()));
        return "stock-points";
    }
    @GetMapping("/vente")
    public String vente(Model model,@RequestParam(defaultValue = "0") int id)throws Exception{
        model.addAttribute("stock",new StockPoints().findById(dataSource.getConnection(),"v_stock_points",id));
        return "vente";
    }
    @PostMapping("/vente/add")
    public String add(@ModelAttribute Vente vente,BindingResult result,RedirectAttributes redirectAttributes)throws Exception{
        if(result.hasErrors()){throw new BadRequestException("vente?id="+vente.getStockPoints().getId(),result);}
        vente.nouveau();
        return "redirect:/user/stock/";
    }
    @GetMapping("/supprimer")
    public String supprimer(@ModelAttribute Vente vente,@RequestParam(defaultValue = "0")int id)throws Exception{
        vente = vente.findById(null,"vente",id);
        vente.executeQuery(dataSource.getConnection(),"UPDATE vente set annule=1 where id_vente="+vente.getId());
        return "redirect:/user/stock/vente/liste";
    }
