...
public class PerteController {
    ...
    public String handleItemException(RedirectAttributes redirectAttributes, Model model, Exception ex) {
        ...
        if (ex instanceof BadRequestException) {
            ...
            if (e.getFunction() != null) {
                ...
                value = "redirect:/user/perte/" + e.getFunction();
            } else {
    ...
    public String renvoye(Model model)throws Exception{
        ...
        model.addAttribute("stock",new ReceptionTransfere().findQuery(null, "select * from v_reception_trans where id_points_vente="+customUser.getUtilisateur().getId_points_vente().getId()));
        return "reception-transfert";
    }

    @PostMapping("/modification")
    public String modification(@ModelAttribute ReceptionTransfere reception, RedirectAttributes redirectAttributes)throws Exception{
        reception.modification();
        return "redirect:/user/perte/";
    }
    @GetMapping("/supprimer")
    public String supprimer(@ModelAttribute ReceptionTransfere reception,@RequestParam(defaultValue = "0") int id)throws Exception{
