...
public class CommissionController {
    ...
    public String handleItemException(RedirectAttributes redirectAttributes, Model model, Exception ex) {
        ...
        if (ex instanceof BadRequestException) {
            ...
            if (e.getFunction() != null) {
                ...
                value = "redirect:/admin/commission/" + e.getFunction();
            } else {
    ...
    public String ram(Model model)throws Exception{
        model.addAttribute("commission",new Commission().find(null,null));
        return "commission";
    }
    @PostMapping("/add")
    public String add(@ModelAttribute Commission ram, BindingResult bindingResult, RedirectAttributes redirectAttributes)throws Exception{
    ...
    public String modifier(@ModelAttribute Commission ram, BindingResult bindingResult,RedirectAttributes redirectAttributes)throws Exception{
        if(bindingResult.hasErrors()){throw new BadRequestException("",bindingResult);}
        ram.update(null);
        return "redirect:/admin/commission/";
    }
    ...
    public String delete(@RequestParam(defaultValue = "0") int id,RedirectAttributes redirectAttributes)throws Exception{
        Commission ram = new Commission().findById(null,"ecran",id);
